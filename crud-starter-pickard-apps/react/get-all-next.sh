...
# Consolidates Next.js project source files into a single markdown file

ROOT_DIR="$(cd "$(dirname "$0")" && pwd)"
NEXTJS_DIR="$ROOT_DIR/crud-app-sqlite-tanstack-shadcn-next"
OUTPUT_DIR="$ROOT_DIR/z_sources"
OUTPUT_FILE="$OUTPUT_DIR/crud-app-sqlite-tanstack-shadcn-next_sources.md"

# Directories to exclude (these won't be descended into at all)
EXCLUDES=("node_modules" ".git" "dist" "build" ".output" ".nuxt" ".cache" ".next" "public")
...
is_included() {
    local file="$1"
    case "$file" in
        # Next.js/React files (TSX, JSX, TS, JS, Styles)
...
is_excluded_dir() {
    ...
    for dir in "${EXCLUDES[@]}"; do
        if [[ "$file" == *"/$dir/"* ]] || [[ "$file" == "$NEXTJS_DIR/$dir"* ]]; then
            return 0
        fi
    done
    return 1
...
# Main execution
echo "=== Next.js Source Code Consolidator ==="
echo "Next.js directory: $NEXTJS_DIR"
echo ""

# Check if Next.js directory exists
if [ ! -d "$NEXTJS_DIR" ]; then
    echo "❌ Error: Next.js directory not found at $NEXTJS_DIR"
    exit 1
fi
...
find "$NEXTJS_DIR" -type f \
    ...
    process_file "$file" "$OUTPUT_FILE" "$NEXTJS_DIR"
done
