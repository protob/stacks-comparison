...
import { projectsApi, ideasApi, songsApi, downloadApi } from '@/api/damApi'
...
import type { Track, Project } from '@/types/dam-api'
...
const loadProjectTracks = async (projectId: string) => {
  loading.value = true
  try {
    project.value = await projectsApi.get(projectId)
    const ideas = await projectsApi.listIdeas(projectId)
...
const unassignTrack = async (track: Track) => {
  const confirmed = await confirmDialogStore.confirm({
    title: 'Remove Track from Project',
    message: `Remove "${track.title || 'Untitled'}" from this project?\n\nThe track will remain in workspace but will be unassigned.`,
    confirmText: 'Remove',
    cancelText: 'Cancel',
    danger: true
  })
  ...
  try {
    ...
    if (route.params.id) {
      await loadProjectTracks(route.params.id as string)
    }
    ...
    await confirmDialogStore.confirm({
      ...
      message: 'Failed to remove track from project. Please try again.',
      confirmText: 'OK',
      danger: false
    })
  }
...
const handleDownload = async () => {
  ...
  try {
    await downloadApi.project(route.params.id as string)
  } catch (error) {
...
<template>
  <div class="h-full flex flex-col">
    <div class="flex items-center justify-between p-4 border-b border-border">
      <div>
        <h1 class="text-xl font-semibold text-text-primary">{{ project?.name || 'Loading...' }}</h1>
        <p class="text-xs text-text-muted">Project Tracks</p>
      </div>
    ...
    >
      ...
      <p class="text-text-muted mb-2">No tracks in this project yet</p>
    </div>
