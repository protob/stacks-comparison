...
import { ideasApi, songsApi, downloadApi } from '@/api/damApi'
...
import type { Track, Idea } from '@/types/dam-api'
...
const loadIdeaTracks = async (ideaId: string) => {
  loading.value = true
  try {
    idea.value = await ideasApi.get(ideaId)
    const songs = await ideasApi.listSongs(ideaId)
    ...
    console.error('Failed to load idea tracks:', error)
  } finally {
    loading.value = false
  }
}
...
const unassignTrack = async (track: Track) => {
  const confirmed = await confirmDialogStore.confirm({
    title: 'Remove Track from Idea',
    message: `Remove "${track.title || 'Untitled'}" from this idea?\n\nThe track will remain in workspace but will be unassigned.`,
    confirmText: 'Remove',
    cancelText: 'Cancel',
    danger: true
  })
  ...
  try {
    ...
    if (route.params.id) {
      await loadIdeaTracks(route.params.id as string)
    }
    ...
    await confirmDialogStore.confirm({
      ...
      message: 'Failed to remove track from idea. Please try again.',
      confirmText: 'OK',
      danger: false
    })
  }
...
const handleDownload = async () => {
  ...
  try {
    await downloadApi.idea(route.params.id as string)
  } catch (error) {
    console.error('Failed to download idea:', error)
    await confirmDialogStore.confirm({
      title: 'Download Failed',
      message: 'Failed to download idea. Please try again.',
      confirmText: 'OK',
      danger: false
    })
  } finally {
...
<template>
  <div class="h-full flex flex-col">
    <div class="flex items-center justify-between p-4 border-b border-border">
      <div>
        <h1 class="text-xl font-semibold text-text-primary">{{ idea?.name || 'Loading...' }}</h1>
        <p class="text-xs text-text-muted">Idea Tracks</p>
      </div>
    ...
    >
      ...
      <p class="text-text-muted mb-2">No tracks in this idea yet</p>
    </div>
